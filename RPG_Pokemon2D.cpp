...
typedef struct {
    ...
    int skillActive;  // 0 = inactive, 1 = active for Barbarian, 2 = active for Technomancer, 3 = active for Rogue
    float skillMultiplier;  // Skill multiplier for damage
    int turnsRemaining;  // Turns remaining for skill effects
} Character;
...
typedef struct {
    ...
    int defence;
    int hasPoison;
    int itemDropChance; // % chance to drop an item
} Enemy;
...
void useItem(Character* player) {
    ...
    // Use the selected item (assuming we only have health potions for now)
    current = player->inventory;
    for (int i = 1; i < choice; i++) {
        current = current->next;
        if (current == NULL) return; // Invalid choice
    }

    if (strcmp(current->name, "Health Potion") == 0) {
        player->hp += 50; // Heal 50 HP
...
void combat(Character *player) {
    int round = 1; // Track rounds of combat
    while (player->hp > 0) {
        printf("Round %d\n", round++);
        Enemy enemy = generateEnemy();
        textDelay("A wild enemy appears!\n", DELAY_MS);

        // Show initial stats for both player and enemy
        ...
        while (player->hp > 0 && enemy.hp > 0) {
            ...
            if (choice == 1) { // Attack
                ...
                // Apply skill multiplier if active
